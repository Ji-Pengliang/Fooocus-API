...
def insert_human(img, mask=None):
    """
    image: uint8 array, H W 3
    mask: uint8 array, H W 3
    """
    arg = generate_from_template(img, mask)
    task = AsyncTask(arg)
    img_inpainting = handler(task)
    
    return img_inpainting

if __name__ == '__main__':
    img = np.array(Image.open('demo.png'), dtype=np.uint8)
    img_inpainting = insert_human(img)
    img_inpainting = Image.fromarray(img_inpainting)
    img_inpainting.save('demo_inpainting.png')